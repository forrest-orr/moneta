...
Suspicion::Suspicion(Entity* Parent, SBlock* Block) {
	//
}

UnsignedModule::UnsignedModule(Entity* Parent, SBlock* Block) : Suspicion(Parent, Block) {}
...
bool Suspicion::InspectEntity(Process &ParentProc, Entity &ParentObj, map<uint8_t*, vector<Suspicion>> &SuspicionsMap) { // Generate suspicions for an entity
	...
	else if (ParentObj.GetType() == Entity::Type::MAPPED_FILE) {
